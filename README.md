...
├── Tutorial_After_you_get_the_data.Rproj  # RStudio project file
│
├── data/
│   ├── raw/                       # Raw data (never modified)
│   │   └── .gitkeep              # Data generated by 00_generate_data.R
│   └── processed/                 # Cleaned, analysis-ready data
│       └── .gitkeep              # Data generated by 01_data_cleaning.R
│
├── scripts/
│   ├── 00_generate_data.R         # Generate simulated dataset
│   ├── 01_data_cleaning.R         # Standalone cleaning script
│   └── run_all.R                  # Run all scripts in sequence
│
├── docs/
...
│
└── renv/                          # R package dependencies (managed by renv)
    └── renv.lock                  # Lockfile for reproducible environments
```

**Note:** The `data/` folders are initially empty. Run `scripts/00_generate_data.R` to generate the simulated raw dataset, then `scripts/01_data_cleaning.R` to create the cleaned dataset.
